function editarlugar(){
    const idLugar = document.getElementById("VLlugarAcambiarnombre").value;
    const nuevoNombre = document.getElementById("VLnombreACambiar").value.trim();

    // Validar que ambos campos estén llenos
    if (!idLugar || !nuevoNombre) {
        Swal.fire({
            title: "Error",
            text: "Selecciona un lugar y escribe un nuevo nombre",
            icon: 'warning'
        });
        return;
    }
    ...
    botones.fire({
        ...
        if(result.isConfirmed){
            document.getElementById("VLlugarAcambiarnombre").value = "";
            document.getElementById("VLnombreACambiar").value = "";

    fetch('/editar_nombre_lugar', {
        method: 'POST',
